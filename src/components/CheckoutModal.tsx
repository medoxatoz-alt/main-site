...
export default function CheckoutModal({ isOpen, onClose, buyNowItem }: { isOpen: boolean; onClose: () => void; buyNowItem?: { productId: string, quantity: number, variantId?: string } | null }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Every fresh open re-fetches (see the effect below) -- show the
      // loading skeleton again rather than a stale render of whatever was
      // last on screen before this same modal instance was closed.
      setLoading(true);
    } else {
      document.body.style.overflow = 'unset';
      // This component stays mounted (isOpen just toggles what it renders),
      // so without this, closing mid-request left `placingOrder` stuck
      // `true` forever -- reopening checkout later showed the Pay button
      // permanently disabled/spinning even though no request was actually
      // in flight anymore.
    ...
    return (
      <div className="fixed inset-0 z-[3000] bg-gray-100 md:bg-black/60 md:backdrop-blur-sm flex flex-col md:items-center md:justify-center p-0 md:p-6 animate-in fade-in duration-200">
        <div className="bg-gray-100 md:bg-[#f8f9fa] w-full h-full md:h-auto md:max-h-[90vh] md:w-full md:max-w-5xl md:rounded-2xl md:shadow-2xl flex flex-col relative overflow-hidden">
          {/* Header */}
          <div className="md:bg-white border-b border-gray-100 px-4 py-4 flex justify-between items-center shrink-0 rounded-t-2xl">
            <h2 className="text-xl font-bold text-gray-900 hidden md:block">Secure Checkout</h2>
            <div className="md:hidden h-6 w-32 bg-gray-200 rounded animate-pulse" />
            <button onClick={onClose} className="text-gray-400 bg-transparent border-none cursor-pointer">
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-0 md:p-6">
            <div className="flex flex-col lg:flex-row gap-2 md:gap-8 items-start">
              <div className="flex-1 w-full flex flex-col gap-2.5 md:gap-6">
                {/* Address block skeleton */}
                <div className="bg-white px-5 py-4 border border-gray-100 rounded-2xl shadow-[var(--shadow-soft)] mx-2 md:mx-0 mt-2 md:mt-0">
                ...
                <div className="bg-white px-5 py-5 border border-gray-100 rounded-2xl shadow-[var(--shadow-soft)] md:hidden mx-2 mt-2 space-y-3">
                ...
                {/* Payment method skeleton (desktop) */}
              ...
              <div className="hidden md:block w-full lg:w-[350px] shrink-0">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-4">
                  <div className="h-5 bg-gray-200 rounded animate-pulse w-2/3 pb-3 border-b border-gray-100" />
                  {[0, 1].map(i => (
                    <div key={i} className="flex justify-between">
          ...
          {/* Sticky bottom button skeleton (mobile) */}
          <div className="md:hidden bg-white px-5 py-5 border-t border-gray-100 shrink-0">
